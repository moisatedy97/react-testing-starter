...
import { getProductsByCategory, productFactory } from "../../moks/db";
...
describe("BrowseProductsPage", () => {
  ...
  beforeAll(() => {
    [1, 2].forEach((item) => {
      const category = productFactory.category.create({ name: "Category " + item });
      categories.push(category);
      [1, 2].forEach(() => {
        products.push(productFactory.product.create({ categoryId: category.id }));
      });
    });
  });
  ...
  afterAll(() => {
    ...
    productFactory.category.deleteMany({
      where: { id: { in: categoryIds } }
    });

    const productIds = products.map((p) => p.id);
    productFactory.product.deleteMany({
  ...
  it("should render all products if All category is selected", async () => {
    ...
    const products = productFactory.product.getAll();
    expectProductsToBeInTheDocument(products);
  });
});
